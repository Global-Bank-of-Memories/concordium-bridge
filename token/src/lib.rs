...
pub const SET_BRIDGE_EVENT_TAG: u8 = 1;
...
struct State<S: HasStateApi> {
    ...
    admin: Address,
    /// The bridge address can mint tokens after signature validation
...
/// A SetBridgeEvent introduced by this smart contract.
#[derive(Serial, SchemaType)]
#[repr(transparent)]
struct SetBridgeEvent {
    /// New bridge address.
    bridge: Address,
}
...
enum WgbmEvent {
    ...
    SetBridge(SetBridgeEvent),
    Cis2Event(Cis2Event<ContractTokenId, ContractTokenAmount>),
}

impl Serial for WgbmEvent {
    fn serial<W: Write>(&self, out: &mut W) -> Result<(), W::Err> {
        match self {
            WgbmEvent::NewAdmin(event) => {
                out.write_u8(NEW_ADMIN_EVENT_TAG)?;
                event.serial(out)
            }
            WgbmEvent::SetBridge(event) => {
                out.write_u8(SET_BRIDGE_EVENT_TAG)?;
                event.serial(out)
            }
            WgbmEvent::Cis2Event(event) => event.serial(out),
        }
    }
}

/// Manual implementation of the `WgbmEventSchema` which includes both the
...
impl schema::SchemaType for WgbmEvent {
    fn get_type() -> schema::Type {
        ...
        event_map.insert(
            SET_BRIDGE_EVENT_TAG,
            (
                "SetBridge".to_string(),
                schema::Fields::Named(vec![(String::from("set_bridge"), Address::get_type())]),
            ),
        );
        event_map.insert(
            TRANSFER_EVENT_TAG,
...
impl<S: HasStateApi> State<S> {
    ...
    /// ***ACHTUNG*** ONLY FOR TESTING PURPOSES
    /// Set the current balance of a given token id for a given address.
    ...
    fn set_balance(
        ...
        ensure_eq!(token_id, &TOKEN_ID_WGBM, ContractError::InvalidTokenId);

        let mut to_state = self.token.entry(*address).or_insert_with(|| AddressState {
            balance: 0u64.into(),
    ...
    fn set_bridge(&mut self, bridge: Option<Address>) -> ContractResult<()> {
        self.bridge = bridge;
...
fn contract_init<S: HasStateApi>(
    ...
    logger.log(&WgbmEvent::Cis2Event(Cis2Event::Mint(MintEvent {
        token_id: TOKEN_ID_WGBM,
        amount: ContractTokenAmount::from(0u64),
        owner: invoker,
    })))?;

    // Log event for where to find metadata for the token
    logger.log(&WgbmEvent::Cis2Event(Cis2Event::TokenMetadata::<_, ContractTokenAmount>(
        TokenMetadataEvent {
            token_id: TOKEN_ID_WGBM,
            metadata_url,
        },
    ...
    logger.log(&WgbmEvent::NewAdmin(NewAdminEvent {
        new_admin: invoker,
    }))?;
...
fn contract_mint<S: HasStateApi>(
    ...
    // Check that bridge is defined in contract.
    if let Some(bridge) = host.state().bridge {
        // Check that only the bridge is authorized to mint tokens.
        ensure_eq!(ctx.sender(), bridge, ContractError::Unauthorized);
    } else {
        // Bridge is not defined.
        bail!(ContractError::Unauthorized);
    }

    ensure_eq!(ctx.sender(), host.state().bridge.unwrap(), ContractError::Unauthorized);
    ...
    let receive_address = params.to;
    let amount = TokenAmountU64::from(params.amount);

    let (state, state_builder) = host.state_and_builder();
    // Update the state.
    state.mint(&TOKEN_ID_WGBM, &amount, &receive_address, state_builder)?;
    ...
    if sender != receive_address {
        logger.log(&WgbmEvent::Cis2Event(Cis2Event::Transfer(TransferEvent {
            token_id: TOKEN_ID_WGBM,
            amount: amount.clone(),
            from: sender,
            to: receive_address,
...
/// Burn an amount of wGBM tokens into CCD
#[receive(
contract = "wGBM",
name = "burn",
...
fn contract_burn<S: HasStateApi>(
    ...
    // Check that bridge is defined in contract.
    if let Some(bridge) = host.state().bridge {
        // Check that only the bridge is authorized to burn tokens.
        ensure_eq!(ctx.sender(), bridge, ContractError::Unauthorized);
    } else {
        // Bridge is not defined.
        bail!(ContractError::Unauthorized);
    ...
    // Update the state.
    state.burn(&TOKEN_ID_WGBM, amount, &params.from)?;

    // Log the burning of tokens.
    logger.log(&WgbmEvent::Cis2Event(Cis2Event::Burn(BurnEvent {
        token_id: TOKEN_ID_WGBM,
        amount,
        owner: params.from,
    })))?;
...
fn contract_set_bridge<S: HasStateApi>(
    ctx: &impl HasReceiveContext,
    host: &mut impl HasHost<State<S>, StateApiType=S>,
    logger: &mut impl HasLogger,
) -> ContractResult<()> {
    ...
    // Parse the parameter.
    let new_bridge: Address = ctx.parameter_cursor().get()?;

    let state = host.state_mut();

    // Update the state.
    state.set_bridge(Some(new_bridge))?;

    // Log a new bridge event.
    logger.log(&WgbmEvent::SetBridge(SetBridgeEvent {
        bridge: new_bridge,
    }))?;
...
/// state-mutative functions (mint, burn, transfer, updateOperator) cannot be
...
fn contract_update_operator<S: HasStateApi>(
    ...
    for param in params {
        ...
        logger.log(&WgbmEvent::Cis2Event(
...
mod tests {
    ...
    const BRIDGE_ACCOUNT: AccountAddress = AccountAddress([4u8; 32]);
    const BRIDGE_ADDRESS: Address = Address::Account(BRIDGE_ACCOUNT);
    const NEW_BRIDGE_ACCOUNT: AccountAddress = AccountAddress([5u8; 32]);
    const NEW_BRIDGE_ADDRESS: Address = Address::Account(NEW_BRIDGE_ACCOUNT);

    // The metadata url for the wGBM token.
    ...
    fn initial_state<S: HasStateApi>(state_builder: &mut StateBuilder<S>) -> State<S> {
        ...
        state
            .mint(&TOKEN_ID_WGBM, &ContractTokenAmount::from(400u64), &ADDRESS_0, state_builder)
            .expect_report("Failed to setup state");
        state.set_bridge(Some(BRIDGE_ADDRESS)).expect_report("Failed to setup state");
        state
    }
    ...
    fn test_init() {
        ...
        let balance0 =
            state.balance(&TOKEN_ID_WGBM, &ADDRESS_0).expect_report("Token is expected to exist");
        claim_eq!(
            balance0,
            0u64.into(),
        ...
        claim!(
            logger.logs.contains(&to_bytes(&WgbmEvent::Cis2Event(Cis2Event::Mint(MintEvent {
                owner:    ADDRESS_0,
                token_id: TOKEN_ID_WGBM,
        ...
        claim!(
            logger.logs.contains(&to_bytes(&WgbmEvent::Cis2Event(Cis2Event::TokenMetadata::<
                _,
                ContractTokenAmount,
            >(
                TokenMetadataEvent {
        ...
        claim!(
            logger.logs.contains(&to_bytes(&WgbmEvent::NewAdmin(NewAdminEvent {
                new_admin: ADDRESS_0,
            }))),
    ...
    fn test_transfer_account() {
        ...
        state
            .mint(&TOKEN_ID_WGBM, &ContractTokenAmount::from(400), &ADDRESS_0, &mut state_builder)
            .expect_report("Failed to setup state");
        let mut host = TestHost::new(state, state_builder);
        ...
        let balance0 = host
            .state()
            .balance(&TOKEN_ID_WGBM, &ADDRESS_0)
            .expect_report("Token is expected to exist");
        let balance1 = host
            .state()
            .balance(&TOKEN_ID_WGBM, &ADDRESS_1)
            .expect_report("Token is expected to exist");
        claim_eq!(
            balance0,
            300.into(),
        ...
        claim_eq!(
            logger.logs[0],
            to_bytes(&WgbmEvent::Cis2Event(Cis2Event::Transfer(TransferEvent {
                from:     ADDRESS_0,
                to:       ADDRESS_1,
                token_id: TOKEN_ID_WGBM,
                amount:   ContractTokenAmount::from(100),
    ...
    #[concordium_test]
    fn test_mint_and_burn_by_non_bridge() {
        // Set up the context.
        let mut ctx = TestReceiveContext::empty();
        ctx.set_sender(ADDRESS_1);
        ...
        // Check the wGBM balance of ADDRESS_1.
        let balance0 = host
            .state()
            .balance(&TOKEN_ID_WGBM, &ADDRESS_1)
            .expect_report("Token is expected to exist");
        claim_eq!(
            ...
            "ADDRESS_1 should not have received wGBM tokens"
        );

        let (state, builder) = host.state_and_builder();

        // Set balance to address
        state.set_balance(
            &TOKEN_ID_WGBM,
            &ADDRESS_1,
        ...
        claim_eq!(
            balance0,
            ContractTokenAmount::from(amount),
            "ADDRESS_1 should have initial amount of WGBM tokens"
        );
    }

    /// Test mint and burn functions.
    #[concordium_test]
    fn test_mint_and_burn_by_bridge() {
        // Set up the context.
        let mut ctx = TestReceiveContext::empty();
        ctx.set_sender(BRIDGE_ADDRESS);
        ...
        claim_eq!(
            balance0,
            ContractTokenAmount::from(amount),
            "ADDRESS_1 should have received wGBM tokens"
        );
        ...
        claim_eq!(
            balance0,
            ContractTokenAmount::from(0),
            "ADDRESS_1 should have no WGBM tokens anymore"
        );
    }
    ...
    fn test_set_bridge() {
        ...
        let parameter_bytes = to_bytes(&[NEW_BRIDGE_ADDRESS]);
        ctx.set_parameter(&parameter_bytes);
        ...
        // Call the contract function.
        let result: ContractResult<()> = contract_set_bridge(&ctx, &mut host, &mut logger);

        // Check the result.
        claim!(result.is_err(), "Results in success");

        // Check contract is paused.
        claim_eq!(host.state().bridge.unwrap(), BRIDGE_ADDRESS, "Smart contract bridge is changed");
        ...
        // Check contract is paused.
        claim_eq!(host.state().bridge.unwrap(), NEW_BRIDGE_ADDRESS, "Smart contract bridge is not changed");
    }
